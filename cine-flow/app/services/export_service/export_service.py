...
from .export_system import ExportSystem, ExportTask, ExportPreset, ExportFormat, ExportQuality
from .jianying_exporter import JianyingDraftGenerator, ProjectToDraftMapper
...
class ExportService:
    ...
    def export_project(self,
        ...
        # 添加视频编辑器相关的元数据
        video_editor = kwargs.get("video_editor")
        if video_editor:
            job.metadata["video_editor"] = video_editor
            # 如果提供了视频编辑器，直接使用其渲染功能
            if mode == ExportServiceMode.SYNC:
                return self._export_with_video_editor(job)
    ...
    def _export_with_video_editor(self, job: ExportJob) -> str:
        """使用视频编辑器导出项目"""
        try:
            # 从作业元数据中获取视频编辑器实例
            video_editor = job.metadata.get("video_editor")
            if not video_editor:
                raise ValueError("Video editor not provided")

            # 获取预设配置
            preset = self.export_system.preset_manager.get_preset(job.preset_id)
            if not preset:
                raise ValueError(f"Preset not found: {job.preset_id}")
            ...
            quality_map = {
                ...
                ExportQuality.ULTRA: "high"
            }
            quality = quality_map.get(preset.quality, "medium")

            # 使用视频编辑器渲染视频
            success = video_editor.render_video(
                output_path=job.output_path,
                quality=quality
            )
            ...
            if success:
                job.metadata["status"] = "completed"
                job.metadata["completed_at"] = asyncio.get_event_loop().time()
                self.logger.info(f"Video exported successfully: {job.output_path}")
            else:
                job.metadata["status"] = "failed"
                job.metadata["error"] = "Video rendering failed"
                self.logger.error(f"Video export failed: {job.output_path}")
    ...
    def export_jianying_draft(self, project_id: str, output_path: str, **kwargs) -> bool:
        """导出剪映草稿"""
        try:
            # 创建项目到草稿的映射器
            mapper = ProjectToDraftMapper()
            
            # 获取视频编辑器服务
            video_service = kwargs.get("video_service")
            if not video_service:
                # 如果没有提供视频服务，使用演示模式
                return self._export_demo_jianying_draft(output_path, **kwargs)
            
            # 获取当前项目
            project = video_service.current_project
            if not project:
                self.logger.error("No current project available")
                return False
            
            # 使用映射器将项目转换为剪映草稿
            project_name = kwargs.get("project_name", project.name if hasattr(project, 'name') else f"Project_{project_id}")
            success = mapper.map_project_to_draft(project, output_path, project_name)
            
            if success:
                self.logger.info(f"Successfully exported Jianying draft: {output_path}")
            else:
                self.logger.error(f"Failed to export Jianying draft: {output_path}")
            
            return success
            
        except Exception as e:
            self.logger.error(f"Error exporting Jianying draft: {e}")
            return False
    
    def _export_demo_jianying_draft(self, output_path: str, **kwargs) -> bool:
        """导出演示用的剪映草稿"""
        try:
            # 创建剪映草稿生成器
            ...
            # 演示：创建一个简单的剪映草稿
            # 添加视频素材
            video_material_id = generator.add_video_material(
                path=kwargs.get("video_path", "/tmp/sample.mp4"),
                name=kwargs.get("video_name", "Sample Video"),
                duration=kwargs.get("video_duration", 10.0)
            )
            
            # 添加音频素材
            audio_material_id = generator.add_audio_material(
                path=kwargs.get("audio_path", "/tmp/sample.mp3"),
                name=kwargs.get("audio_name", "Sample Audio"),
                duration=kwargs.get("audio_duration", 10.0)
            )
            ...
            # 将素材添加到轨道
            generator.add_material_to_track(video_track_id, video_material_id)
            generator.add_material_to_track(audio_track_id, audio_material_id)
            
            # 生成剪映草稿
            success = generator.generate_draft(
                project_name=kwargs.get("project_name", f"Demo_Project_{int(time.time())}"),
                output_path=output_path,
            ...
            if success:
                self.logger.info(f"Successfully exported demo Jianying draft: {output_path}")
            else:
                self.logger.error(f"Failed to export demo Jianying draft: {output_path}")
            
            return success
            
        except Exception as e:
            self.logger.error(f"Error exporting demo Jianying draft: {e}")
            return False
    
    def export_draft_from_project(self, project, output_path: str, project_name: str = None) -> bool:
        """直接从项目对象导出剪映草稿"""
        try:
            # 创建项目到草稿的映射器
            mapper = ProjectToDraftMapper()
            ...
            if success:
                self.logger.info(f"Successfully exported Jianying draft from project: {output_path}")
            else:
                self.logger.error(f"Failed to export Jianying draft from project: {output_path}")
            
            return success
            
        except Exception as e:
            self.logger.error(f"Error exporting Jianying draft from project: {e}")
            return False
