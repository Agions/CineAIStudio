...
class JianyingDraftGenerator:
    ...
    def add_video_material(self,
                          ...
                          start_time: float = 0.0,
                          end_time: float = None,
                          skip_exists_check: bool = False) -> str:
        """添加视频素材"""
        try:
            if not skip_exists_check and not os.path.exists(path):
...
class ProjectToDraftMapper:
        ...
        try:
            ...
            for clip in project.video_clips + project.audio_clips:
                ...
                if media_item.id not in material_map:
                    ...
                    material_map[media_item.id] = material_id

            # 2. 创建轨道
            video_track_id = self.generator.create_track("video")
            audio_track_id = self.generator.create_track("audio")

            # 3. 添加视频片段到视频轨道
            for video_clip in project.video_clips:
                material_id = material_map.get(video_clip.media_item.id)
                if material_id:
                    self.generator.add_material_to_track(
                        track_id=video_track_id,
                        material_id=material_id,
                        start_time=video_clip.start_time
                    )
                    # 添加特效
                    self._map_effects(video_clip.effects, video_clip.start_time, video_clip.end_time - video_clip.start_time)

            # 4. 添加音频片段到音频轨道
            for audio_clip in project.audio_clips:
                material_id = material_map.get(audio_clip.media_item.id)
                if material_id:
                    self.generator.add_material_to_track(
            ...
            # 5. 生成Draft文件
            resolution = (project.width, project.height) if hasattr(project, 'width') and hasattr(project, 'height') else (1920, 1080)
            fps = project.fps if hasattr(project, 'fps') else 30.0

            success = self.generator.generate_draft(
            ...
            if success:
                self.logger.info(f"Mapped project to draft: {output_path}")
            else:
                self.logger.error(f"Failed to map project to draft: {output_path}")

            return success

        except Exception as e:
            self.logger.error(f"Failed to map project to draft: {e}")
            return False

    def _map_effects(self, effects: List[Dict[str, Any]], start_time: float, duration: float) -> None:
        """映射特效"""
        for effect in effects:
            effect_name = effect.get('name')
            if effect_name:
                # 将CineAIStudio特效映射为剪映特效
    ...
    def _map_effect_type(self, effect_name: str) -> str:
        """映射特效类型"""
        effect_type_map = {
            'grayscale': 'color',
            'blur': 'filter',
    ...
    def map_draft_to_project(self, draft_data: Dict[str, Any]) -> Dict[str, Any]:
        """将剪映Draft映射为CineAIStudio项目数据（简化版）"""
        try:
            # 这里实现从剪映Draft到CineAIStudio项目的映射
            # 目前只返回基础项目信息，完整映射需要更复杂的逻辑
            project_data = {
                "name": draft_data.get("project_info", {}).get("name", "Imported_Jianying_Project"),
                "resolution": draft_data.get("settings", {}).get("resolution", {"width": 1920, "height": 1080}),
                "fps": draft_data.get("settings", {}).get("fps", 30.0),
                "duration": draft_data.get("settings", {}).get("duration", 0.0),
                "materials": draft_data.get("materials", []),
                "tracks": draft_data.get("tracks", [])
            }

            self.logger.info("Mapped draft to project data")
            return project_data

        except Exception as e:
            self.logger.error(f"Failed to map draft to project: {e}")
            raise
...
class JianyingDraftParser:
    ...
    def validate_draft(self, draft_data: Dict[str, Any]) -> bool:
        ...
        if version not in self.supported_versions:
            self.logger.warning(f"Draft version {version} not fully supported. Supported versions: {', '.join(self.supported_versions)}")

        # 检查轨道结构
        for track in draft_data.get("tracks", []):
            if "id" not in track:
                self.logger.error("Missing track id")
                return False
            if "type" not in track:
                self.logger.error("Missing track type")
                return False

        # 检查素材结构
        for material in draft_data.get("materials", []):
            if "id" not in material:
                self.logger.error("Missing material id")
                return False
            if "type" not in material:
                self.logger.error("Missing material type")
                return False
            if "path" not in material:
                self.logger.error("Missing material path")
                return False

        return True

    def analyze_draft_format(self, draft_data: Dict[str, Any]) -> Dict[str, Any]:
        """分析剪映草稿格式结构"""
        analysis = {
            "version": draft_data.get("version"),
            "is_valid": self.validate_draft(draft_data),
            "project_info": draft_data.get("project_info", {}),
            "settings": draft_data.get("settings", {}),
            "materials_count": len(draft_data.get("materials", [])),
            "tracks_count": len(draft_data.get("tracks", [])),
            "effects_count": len(draft_data.get("effects", [])),
            "text_overlays_count": len(draft_data.get("text_overlays", [])),
        ...
        for material in materials:
            ...
            material_types[m_type] = material_types.get(m_type, 0) + 1
        analysis["material_types"] = material_types
        ...
        analysis["track_types"] = track_types
    ...
    def get_materials_by_type(self, draft_data: Dict[str, Any], material_type: str) -> List[Dict[str, Any]]:
        """按类型获取素材列表"""
        return [m for m in draft_data.get("materials", []) if m.get("type") == material_type]

    def get_tracks(self, draft_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """获取轨道列表"""
        return draft_data.get("tracks", [])

    def get_tracks_by_type(self, draft_data: Dict[str, Any], track_type: str) -> List[Dict[str, Any]]:
        """按类型获取轨道列表"""
        return [t for t in draft_data.get("tracks", []) if t.get("type") == track_type]

    def get_effects(self, draft_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """获取特效列表"""
        return draft_data.get("effects", [])

    def get_text_overlays(self, draft_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """获取文字叠加列表"""
        return draft_data.get("text_overlays", [])
    ...
    def generate_draft_report(self, draft_path: str, output_path: str) -> bool:
        """生成剪映草稿分析报告"""
        try:
            draft_data = self.parse_draft(draft_path)
            analysis = self.analyze_draft_format(draft_data)

            with open(output_path, 'w', encoding='utf-8') as f:
                f.write("# 剪映草稿分析报告\n")
                f.write(f"# 生成时间: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write(f"# 草稿路径: {draft_path}\n\n")

                # 基本信息
                f.write("## 基本信息\n")
                f.write(f"- 版本: {analysis.get('version')}\n")
                f.write(f"- 格式有效: {'是' if analysis.get('is_valid') else '否'}\n")
                f.write(f"- 项目名称: {analysis.get('project_info', {}).get('name', '未指定')}\n")
                f.write(f"- 创建者: {analysis.get('project_info', {}).get('created_by', '未指定')}\n\n")

                # 设置信息
                f.write("## 项目设置\n")
                settings = analysis.get('settings', {})
                f.write(f"- 分辨率: {settings.get('resolution', {}).get('width', 0)}x{settings.get('resolution', {}).get('height', 0)}\n")
                f.write(f"- 帧率: {settings.get('fps', 0)} fps\n")
                f.write(f"- 音频采样率: {settings.get('audio_sample_rate', 0)} Hz\n")
                f.write(f"- 项目时长: {settings.get('duration', 0)} 秒\n\n")

                # 素材信息
                f.write("## 素材统计\n")
                f.write(f"- 总素材数: {analysis.get('materials_count')}\n")
                for m_type, count in analysis.get('material_types', {}).items():
                    f.write(f"  - {m_type}: {count}\n")
                f.write(f"- 总轨道数: {analysis.get('tracks_count')}\n")
                for t_type, count in analysis.get('track_types', {}).items():
                    f.write(f"  - {t_type}: {count}\n")
                f.write(f"- 总特效数: {analysis.get('effects_count')}\n")
                f.write(f"- 总文字叠加数: {analysis.get('text_overlays_count')}\n\n")

            self.logger.info(f"Generated draft report: {output_path}")
            return True

        except Exception as e:
            self.logger.error(f"Failed to generate draft report: {e}")
            return False