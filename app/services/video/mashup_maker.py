...
class MashupProject:
    ...
    source_video: str = ""  # 兼容 BaseProject
    video_duration: float = 0.0  # 兼容 BaseProject
    scenes: List[SceneInfo] = field(default_factory=list)  # 兼容 BaseProject

    # 分析结果
    all_clips: List[ClipInfo] = field(default_factory=list)
    selected_clips: List[ClipInfo] = field(default_factory=list)
...
class MashupMaker(ProgressMixin):
    """
    AI 视频混剪制作器
